...
function createTicketsRouter(database = pool) {
  ...
  router.get("/", async (request, response, next) => {
    ...
    if (search)
      query += ` AND (t.subject ILIKE '%${search}%' OR t.description ILIKE '%${search}%')`;
  ...
  router.get("/:id/edit", async (request, response, next) => {
    try {
      ...
      const ticket = await database.query(
        `SELECT * FROM tickets WHERE id = ${request.params.id}`,
      );
      const assets = await database.query(
        "SELECT id, name FROM assets ORDER BY name",
      );
  ...
  router.post("/:id/close", async (request, response, next) => {
    try {
      ...
      // [VULN-005][A01:CSRF][CWE-352] No CSRF token is checked.
      await database.query(
        `UPDATE tickets SET status = 'closed', updated_at = CURRENT_TIMESTAMP WHERE id = ${request.params.id}`,
      );
      response.redirect(`/tickets/${request.params.id}`);
    } catch (error) {
      next(error);
  ...
  router.post("/:id/delete", async (request, response, next) => {
    try {
      ...
      // [VULN-005][A01:CSRF][CWE-352] No CSRF token is checked.
      await database.query(
        `DELETE FROM tickets WHERE id = ${request.params.id}`,
      );
  ...
  router.get("/:id", async (request, response, next) => {
    try {
      ...
      const result =
        await database.query(`SELECT t.*, u.username AS requester_username, a.name AS asset_name
        FROM tickets t
        JOIN users u ON u.id = t.requester_id
        LEFT JOIN assets a ON a.id = t.asset_id
